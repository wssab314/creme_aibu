						...
						<view class="group-bottom-right acea-row row-column row-between">
							...
							<view class="right-bottom acea-row  row-between">
								<view class="price">
									...
									<view class="num otNum regular"  v-if="typeShow.includes(3)" :style="[originalColor]"><text
											class="num-title"></text><text
				...
				<view v-for="(item, index) in groupProductList" :key="index" class="three-box"
					...
					<view class="group-bottom-left">
						<view class="img acea-row row-center row-middle three-img">
							<easy-loadimage class="loadimage"  width="100%" height="100%"  :image-src="item.image"  :radius="dataConfig.contentStyle.val"></easy-loadimage>
						</view>
					</view>
					<view class="two-item">
						<view class="title acea-row">
						...
						<view class="two-item-bottom">
							<view class="price">
								<view class="pinkNum line1" v-if="typeShow.includes(2)" :style="[priceColor]">
									<text class="num-icon"><text class="min-text">低至</text>￥</text><text :style="[{'font-size':item.minPrice.length>6?'30rpx':'36rpx'}]" class="num semiBold">{{item.minPrice}}</text>
								</view>
								<view class="otNum regular" v-if="typeShow.includes(3)" :style="[originalColor]">
									￥{{item.price}}
								</view>
					...
					<view v-for="(item, index) in groupProductList" :key="index" class="four-item" :style="[fourStyle]"
						...
						<view class="group-bottom-left">
							<view class="img acea-row row-center row-middle four-img">
								<easy-loadimage :image-src="item.image" width="214rpx"
									height="214rpx" :radius="dataConfig.contentStyle.val"></easy-loadimage>
							</view>
						</view>
						<view class="two-item">
		...
		computed: {
			...
			peopleStyle() {
				return [{
					...
					{
						background: `linear-gradient(to right,${this.dataConfig.groupTitleBgColor.color[0].item}, ${this.dataConfig.groupTitleBgColor.color[1].item})`,
					},
					{
						'align-self': 'baseline',
					},
