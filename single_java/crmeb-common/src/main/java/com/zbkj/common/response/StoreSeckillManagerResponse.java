...
public class StoreSeckillManagerResponse {
    ...
    public Integer getKillStatus() {
        ...
        if(status == 1 && CrmebDateUtil.getCurrentHour() < startTime){
            _killStatus = 1;
        }
        else if(status == 0) {
            _killStatus = 0;
        }
        else if(status == 1 && CrmebDateUtil.getCurrentHour() >= startTime
                && CrmebDateUtil.getCurrentHour() < endTime) {
            _killStatus = 2;
        }
        else if(status == 1 && CrmebDateUtil.getCurrentHour() >= endTime) {
            _killStatus = -1;
        }
        return _killStatus;
    }
