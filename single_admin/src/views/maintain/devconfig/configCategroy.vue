  ...
  <div class="divBox">
    <el-card class="box-card">
      <div slot="header">
        ...
        <el-alert title="温馨提示" type="success" description="添加一级分类以后，务必添加二级分类并配置表单，否则会出现渲染错误">
        </el-alert>
      </div>
      <el-table ref="treeList" :data="treeList" style="width: 100%" row-key="id" size="mini" class="table"
        highlight-current-row :tree-props="{ children: 'child', hasChildren: 'hasChildren' }">
        ...
        <el-table-column label="操作" min-width="250" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" size="small" :disabled="scope.row.pid > 0"
              @click="handlerOpenAdd(scope.row)">添加子目录</el-button>
    ...
    <el-dialog :title="editDialogConfig.isCreate === 0 ? '添加分类' : '编辑分类'" :visible.sync="editDialogConfig.visible"
    ...
    <el-dialog title="选择已配置的表单" :visible.sync="configFormSelectedDialog.visible">
      ...
      <form-config-list v-if="configFormSelectedDialog.visible" select-model
