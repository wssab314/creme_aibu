<template>
  <div class="divBox">
    <el-card :bordered="false" shadow="never" class="ivu-mt el-card__header" :body-style="{ padding: 0 }">
      <div class="padding-add">
        <el-form ref="tableFrom" :model="tableFrom" size="small" :inline="true" @submit.native.prevent>
          <el-form-item label="创建时间：" prop="date">
            <el-date-picker style="width: 260px" v-model="timeVal" type="datetimerange" range-separator="-"
              value-format="yyyy-MM-dd HH:mm:ss" start-placeholder="开始日期" end-placeholder="结束日期" @change="onchangeTime"
              :default-time="['00:00:00', '23:59:59']">
            </el-date-picker>
          </el-form-item>
          <el-form-item label="活动状态：" prop="status">
            <el-select v-model="tableFrom.runningStatus" placeholder="请选择" class="selWidth" clearable
              @change="getList(1)">
    ...
    <el-card class="box-card mt14" :body-style="{ padding: '20px' }" :bordered="false" shadow="never">
      ...
      <el-table :data="tableData.data" style="width: 100%" class="mt20" size="small" highlight-current-row>
        ...
        <el-table-column label="是否开启" fixed="right" min-width="90">
          <template slot-scope="scope">
            <el-switch v-if="checkPermi(['admin:activitystyle:updatestatus'])" v-model="scope.row.status"
        ...
        <el-table-column label="操作" width="100" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button class="mr5" type="text" size="small" @click="onEdit(scope.row)">编辑</el-button>
            <el-button type="text" size="small" @click="handleDelete(scope.row.id, scope.$index)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
        <el-pagination background :page-sizes="[20, 40, 60, 80]" :page-size="tableFrom.limit"
          :current-page="tableFrom.page" layout="total, sizes, prev, pager, next, jumper" :total="tableData.total"
